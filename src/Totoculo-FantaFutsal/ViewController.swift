...
class ViewController: UIViewController, WKNavigationDelegate, UIDocumentInteractionControllerDelegate {
    ...
    override func viewDidLayoutSubviews() {
        ...
        Totoculo_FantaFutsal.webView.frame = calcWebviewFrame(webviewView: webviewView, toolbarView: nil)
    }
    
    @objc func keyboardWillHide(_ notification: NSNotification) {
        Totoculo_FantaFutsal.webView.setNeedsLayout()
    }
    
    func initWebView() {
        Totoculo_FantaFutsal.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(Totoculo_FantaFutsal.webView);
        
        Totoculo_FantaFutsal.webView.uiDelegate = self;
        
        Totoculo_FantaFutsal.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)

        if(pullToRefresh){
            let refreshControl = UIRefreshControl()
            refreshControl.addTarget(self, action: #selector(refreshWebView(_:)), for: UIControl.Event.valueChanged)
            Totoculo_FantaFutsal.webView.scrollView.addSubview(refreshControl)
            Totoculo_FantaFutsal.webView.scrollView.bounces = true
        }

        if #available(iOS 15.0, *), adaptiveUIStyle {
            themeObservation = Totoculo_FantaFutsal.webView.observe(\.themeColor) { [unowned self] webView, _ in
                let backgroundColor = Totoculo_FantaFutsal.webView.underPageBackgroundColor;
                let themeColor = Totoculo_FantaFutsal.webView.themeColor;
    ...
    @objc func refreshWebView(_ sender: UIRefreshControl) {
        Totoculo_FantaFutsal.webView?.reload()
...
//        toolbarView.autoresizingMask = [.flexibleTopMargin, .flexibleRightMargin, .flexibleWidth]
    ...
    func overrideUIStyle(toDefault: Bool = false) {
        if #available(iOS 15.0, *), adaptiveUIStyle {
            if (((htmlIsLoaded && !Totoculo_FantaFutsal.webView.isHidden) || toDefault) && self.currentWebViewTheme != .unspecified) {
                UIApplication
                    .shared
                    .connectedScenes
    ...
    @objc func loadRootUrl(cachePolicy: NSURLRequest.CachePolicy = .useProtocolCachePolicy) {
        Totoculo_FantaFutsal.webView.load(URLRequest(url: SceneDelegate.universalLinkToLaunch ?? SceneDelegate.shortcutLinkToLaunch ?? rootUrl, cachePolicy: cachePolicy))
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            Totoculo_FantaFutsal.webView.isHidden = false
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                Totoculo_FantaFutsal.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(Totoculo_FantaFutsal.webView.estimatedProgress);
    ...
    deinit {
        Totoculo_FantaFutsal.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "print" {
            printView(webView: Totoculo_FantaFutsal.webView)
        }
